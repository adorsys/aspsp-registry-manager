...
public class AspspServiceImplTest {
    ...
    public void convertAndSaveAll() {
        List<AspspPO> aspspPOS = Collections.singletonList(po);
        List<AspspBO> aspspBOS = Collections.singletonList(bo);

        when(converter.toAspspPOList(aspspBOS)).thenReturn(aspspPOS);
        doNothing().when(repository).saveAll(aspspPOS);

        aspspService.saveAll(aspspBOS);

        verify(converter, times(1)).toAspspPOList(aspspBOS);
        verify(repository, times(1)).saveAll(captor.capture());

        assertEquals(aspspPOS.size(), captor.getValue().size());
        assertThat(aspspPOS.get(0), is(captor.getValue().get(0)));
    }
}
